...
class KuramotoSivashinskyPINN(PINN):
    def __init__(self, 
                 data: dict , 
                 ...
                 log_dir: str = "./tb_logs/KuramotoSivashinsky"):
    ...
    def phys_loss(self):
        """ Loss for Kuramoto-Sivashinsky equation """
        u_f = self.forward(torch.cat([self.x_f, self.t_f], dim=-1))

        u_t = torch.autograd.grad(u_f.sum(), self.t_f, create_graph=True)[0]
        u_x1 = torch.autograd.grad(u_f.sum(), self.x_f, retain_graph=True, create_graph=True)[0]
        u_x2 = torch.autograd.grad(u_x1.sum(), self.x_f, retain_graph=True, create_graph=True)[0]
        u_x3 = torch.autograd.grad(u_x2.sum(), self.x_f, retain_graph=True, create_graph=True)[0]
        u_x4 = torch.autograd.grad(u_x3.sum(), self.x_f, retain_graph=True, create_graph=True)[0]

        lambda_1, lambda_2 = self.get_params()
        eq = u_t + lambda_1 * u_f * u_x1 + lambda_2 * u_x2 + u_x4
...
class KuramotoSivashinskyBVPPINN(KuramotoSivashinskyPINN):
    def __init__(self, 
                 data: dict , 
                 layers: list = [2, 50, 50, 50, 50, 1], 
                 lr: float = 1e0,
...
class KuramotoSivashinskyParameterPINN(KuramotoSivashinskyPINN):
    def __init__(self, 
                 data: dict , 
                 layers: list = [2, 50, 50, 50, 50, 1], 
                 lr: float = 1e0,
